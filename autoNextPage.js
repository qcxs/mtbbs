...
// @version      1.0
// @description  集众多页面为一体，统一使用page参数控制分页，适配所有帖子列表。滚动加载与页码跳转，地址栏无刷新更新。已适配：导读、社区、搜索、个人空间、帖子评论、
// @author       青春向上
...
(function () {
    ...
    function getPageInfo() {
        ...
        const currentPage = parseInt(urlPage?.trim()) || ($select.length ? parseInt($select.find('option:selected').val()) : 1);
        //打印日志
    ...
    function addPageMarker({ pageNum = currentPage, isLoadingState, errorObject, loadAll }) {
        ...
        if (isLoadingState) {
            ...
            marker = $(`<li class="retry-marker" style="text-align:center; padding: 0; margin:10px 0; color:red;">
             ...
             <a href="javascript:;" style="color:#007bff; margin-left:5px;">点击重试</a>
             <p style="color:red; margin-left:5px;">${JSON.stringify(errorObject)}</p>
        </li>`)
            // 绑定重试事件，同理可添加一些点击事件
            ...
            marker = $(`<li style="text-align:center; padding: 0; margin:10px 0; color:#666;">
                    已全部 <span class="page-jump-link" style="color:#507daf;">${totalPageText}</span> 
            ...
            $(marker).on('click', '.loadPreNext', function () {
                if (isFailed) {
                    alert('有未重试的页码，请重试后再点击。')
                    return;
                }
                loadPage(pageNum - 1);
                $(this).remove();
                console.log(`尝试加载${pageNum - 1}第页`)
            });
        }
    ...
    function loadPage(page) {
        ...
        $.ajax({
            ...
            if (root.includes('本版块或指定的范围内尚无主题') || (!htmlContent && !root.includes('下一页') && totalPage == ENUM.unknownPage)) {
                //下一页无数据，直接判定已全部加载
                AllLoaded()
                return;
            }
    ...
    function initCurrentPage() {
        ...
        //检测是否全部加载
        if (currentPage == totalPage) AllLoaded();
